...
#
describe "AMQ::Client::Async::CoolioClient", "Connection.Start", :nojruby => true do
  ...
  context "with valid credentials" do
    it "should trigger the callback" do
      coolio do
        AMQ::Client::Async::CoolioClient.connect do |client|
  ...
  context "with invalid credentials" do
    context "when given an errback" do
      it "should trigger the errback" do
        coolio do
          AMQ::Client::Async::CoolioClient.connect(:port => 12938, :on_tcp_connection_failure => proc { done }) do |client|
    ...
    context "when given no errback" do
      it "should raise an error" do
        expect {
          coolio do
            begin
              AMQ::Client::Async::CoolioClient.connect(:port => 12938) { }
              done
            rescue Exception => e
              done(0.5)
            end
